...
__global__ void gemm_tiled32_kernel(const float* A, const float* B, float* C, int M, int N, int K)
{
...
static void launch_gemm_tiled32(const float* d_A, const float* d_B, float* d_C, const GemmProblem& p)
{
    dim3 block(TILE_DIM, TILE_DIM);
    dim3 grid((p.N + TILE_DIM - 1) / TILE_DIM, (p.M + TILE_DIM - 1) / TILE_DIM);
    gemm_tiled32_kernel<<<grid, block>>>(d_A, d_B, d_C, p.M, p.N, p.K);
}
...
int main(int argc, char** argv)
{
    ...
    print_run_config("CUDA tiled32 GEMM", config);
    ...
    for (int i = 0; i < config.warmup; ++i) {
        launch_gemm_tiled32(d_A, d_B, d_C, p);
    }
