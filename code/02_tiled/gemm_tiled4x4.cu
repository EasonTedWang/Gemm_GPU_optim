...
__global__ void gemm_tiled4x4_kernel(const float* A, const float* B, float* C, int M, int N, int K)
{
    __shared__ float sharedA[BLOCK_TILE_M][BLOCK_TILE_K + 1];
    __shared__ float sharedB[BLOCK_TILE_K][BLOCK_TILE_N + 1];
...
static void launch_gemm_tiled4x4(const float* d_A, const float* d_B, float* d_C, const GemmProblem& p)
{
    dim3 block(THREADS_N, THREADS_M);
    dim3 grid((p.N + BLOCK_TILE_N - 1) / BLOCK_TILE_N,
              (p.M + BLOCK_TILE_M - 1) / BLOCK_TILE_M);
    gemm_tiled4x4_kernel<<<grid, block>>>(d_A, d_B, d_C, p.M, p.N, p.K);
}
...
int main(int argc, char** argv)
{
    ...
    print_run_config("CUDA tiled4x4 GEMM", config);
    ...
    for (int i = 0; i < config.warmup; ++i) {
        launch_gemm_tiled4x4(d_A, d_B, d_C, p);
    }
