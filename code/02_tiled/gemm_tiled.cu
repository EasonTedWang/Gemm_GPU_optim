...
static void launch_gemm_tiled(const float* d_A, const float* d_B, float* d_C, const GemmProblem& p)
{
    dim3 block(TILE_DIM, TILE_DIM);
    dim3 grid((p.N + TILE_DIM - 1) / TILE_DIM, (p.M + TILE_DIM - 1) / TILE_DIM);
    gemm_tiled_kernel<<<grid, block>>>(d_A, d_B, d_C, p.M, p.N, p.K);
}
...
int main(int argc, char** argv)
{
    ...
    print_run_config("CUDA tiled GEMM", config);
